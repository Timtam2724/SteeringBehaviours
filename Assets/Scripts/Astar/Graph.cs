...
public class Graph : MonoBehaviour
{
    ...
    void OnDrawGizmos()
    {
        ...
        // Check if nodes have been creates
        if (nodes != null)
        {
    ...
    // Genertes a 2D grid on the X and Z axis
    public void CreateGrid()
    {
        ...
        for (int x = 0; x < gridSizeX; x++)
        {
            for (int z = 0; z < gridSizeZ; z++)
            {
                ...
                // Create the node and put it in the array
    ...
    public Node GetNodeFromPosition(Vector3 position)
    {
        ...
        //Translate percentage tp x & z coordinates
        int x = Mathf.RoundToInt((gridSizeX - 1) * percentX);
        int z = Mathf.RoundToInt((gridSizeZ - 1) * percentZ);

        //Return the node at translated coordinates
        return nodes[x, z];
    }

    // This function will return all nieghbors
    public List<Node> GetNeighbours(Node node)
    {
        // Make a new list of nieghbours
        ...
        for (int x = -1; x <= 1; x++)
        {
            for (int z = -1; z <= 1; z++)
            {
                // Check if the coordinates is the current node
                if (x == 0 && z == 0)
